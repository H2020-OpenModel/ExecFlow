...
from aiida.orm import Data, Dict, List, SinglefileData, Str, to_aiida_type
    ...
    def setup(self):
        ...
        for k, f in self.inputs.files.items():
            if "node" in f:
                ...
                self.ctx.filenodes[k] = fill_template(
                    SinglefileData(f["template"]), f.get("parameters", Dict())
                )
