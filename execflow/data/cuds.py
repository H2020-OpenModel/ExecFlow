...
from oteapi.models import AttrDict, SessionUpdate, FunctionConfig
...
def gen_property(props, dims, dimnames, dim, curid, attributename, attribute, attributes_dict):
    ...
    if isinstance(attribute, list):
        ...
        if dim == 1:
            ...
            dimnames[attributename] = [dlite.Dimension(f'{attributename}_{dim}', f'Dimension {dim} of attribute {attributename}')]
        elif dim > len(dims[attributename]):
            # Next dimension -> just append a new dimension
            dims[attributename].append(len(attribute))
            dimnames[attributename].append(dlite.Dimension(f'{attributename}_{dim}', f'Dimension {dim} of attribute {attributename}'))
        ...
        for name in attribute:
            ...
            gen_property(props, dims, dimnames, dim, curid, propname, attribute[name], attributes_dict)
        ...
        if not any(x.name == attributename for x in props):
            ...
            if dim > 0:
                props.append(dlite.Property(attributename, attype_name, [x.name for x in dimnames[attributename]], None, None))
            else:
